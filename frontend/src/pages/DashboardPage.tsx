...
import type { CategoryBudget, DashboardMetrics, ScheduledIncome } from '../api/types'
...
export function DashboardPage({ onConnectionChange }: { onConnectionChange: (connected: boolean) => void }) {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const [health, dashboard, budgetData, incomeData] = await Promise.all([api.health(), api.dashboard(month), api.budgets(month), api.scheduledIncomes.list(month)])
      onConnectionChange(health.status === 'ok'); setMetrics(dashboard); setBudgets(budgetData); setScheduledIncomes(incomeData)
    } catch (requestError) {
