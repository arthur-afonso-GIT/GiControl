...
import type { CategoryBudget, DashboardMetrics } from '../api/types'
...
export function DashboardPage({ onConnectionChange }: { onConnectionChange: (connected: boolean) => void }) {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const [health, dashboard, budgetData] = await Promise.all([api.health(), api.dashboard(month), api.budgets(month)])
      onConnectionChange(health.status === 'ok'); setMetrics(dashboard); setBudgets(budgetData)
    } catch (requestError) {
